...
#### 4. 現場調査と本番版の確定（2026-06-25 午後 — SSH接続して実施）
- **顧客報告「古いExcelで作成したものが反映されない」= バグではなくタイミング**。未反映14件は全て本日13:25〜13:37に追加（ctime確認）、直近同期12:05の後・次回15:00の前で待機中なだけ。14件とも正規OOXML(PKzip)・`輸入書類`内（同フォルダは42件中28件同期済）。同期は times mode で12:00/15:00の1日2回 → 昼以降追加分は15:00反映。Excelバージョンは無関係（顧客の誤帰属）。memory `feedback_verify_customer_attributed_cause`。
- **本番が動かしているのは session-10版（2026-05-25）と確定**（実機の loaded module を inspect 検証）。site-packages の `watcher.py` は **04-16以降未更新**・`main.py`は05-25、**削除伝播なし・rglobのまま**。**セッション11の「本番デプロイ済み」も実機未到達**（06-24の .bak が存在しない）。git コミット消失と同様、デプロイも別コピー側で完結し実機に届かなかった。memory `reference_verify_prod_deploy_state`。
  - ※本ブロックで先に書いた「現場は素のプラン版（削除伝播）稼働」は**誤り**。正しくは **session-10版＝削除伝播なし**。
  - 含意: NAS削除→Drive追従は**本番で未稼働**（削除しても旧スプレッドが残る顧客報告は本番では未解決）。ただし今日の症状とは無関係。

#### 5. 次回最優先タスク
- **🔴 15:00同期の結果確認**: 現場の14件（輸入書類）が15:00同期で tracked＋Sheets生成 されたか実機確認（タイミング診断の事実確認 closing）。SSHヘルパー方式は本セッションで確立（scratchやり方は memory 参照）。
- **🔴 ハードン版を現場へ再デプロイ**: 本番 session-10 → 正本 `c9a7f87`（削除伝播＋全ハードニング）へ更新。`.bak`退避→site-packages差し替え→`nssm restart tamatex`→1サイクルのログ確認。**clobber注意: 手動差し替えは後の `pip install .` で巻き戻るため、できれば `pip install` で再現可能に**。
- **🟡 同期頻度の検討**: 顧客は日中ファイルを随時追加 → 1日2回だとラグ大。9/11/13/15/17時 等への増回を提案検討（config＋`nssm restart`）。
